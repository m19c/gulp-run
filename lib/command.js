	...
	this.exec = function exec(stdin, callback) {
		...
		if (isWindows) {
			if (opts.usePowerShell) {
				...
				cmdargs.unshift('/c')
				subshell = childProcess.spawn('cmd.exe', cmdargs, {env:opts.env, cwd:opts.cwd});
			}
		} else {
...
// This is rewuired for cmd.exe on Windows.
var splitCommand = function (command) {
	var argregexp = /(".*?")|('.*?')|([^\s]+)/;
	var args = [];
	while (command.length > 0) {
		var match = argregexp.exec(command);
		var arg = match[0]
		args.push(arg);
		command = command.substring(arg.length + match.index);
	}
	return args;
}
